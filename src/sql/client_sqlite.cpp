...
bool SQLiteClient::opendb(const string& dbName, int flag /* = SQLITE_OPEN_READWRITE | SQLITE_OPEN_CREATE*/) {
	int ret = sqlite3_open_v2(dbName.c_str(), &m_db, SQLITE_OPEN_READWRITE | SQLITE_OPEN_CREATE, NULL);
	return !ret;
}
...
bool SQLiteClient::update(const string& sql) {
	if (sqlite3_exec(m_db, sql.c_str(), sqlite_exec_callback, nullptr, nullptr)) {
		return false;
	}
	return true;
}
